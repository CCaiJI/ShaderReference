...
namespace MG.MDV
{
    public class MarkdownViewer
    {
        ...
        public void Draw(Rect rectBg,Rect rectView)
        {
            GUI.skin    = mSkin;
            GUI.enabled = true;
            ...
            // clear background
            GUI.DrawTexture( rectBg, EditorGUIUtility.whiteTexture, ScaleMode.StretchToFill, false );
            var rectContainer = rectView;// GetEditorRect();
          
  
            // scroll window

            var padLeft     = rectView.x;
            var padRight    = 4.0f;
            var padHoriz    = padLeft + padRight;
            var scrollWidth = GUI.skin.verticalScrollbar.fixedWidth;
            var minWidth    = rectContainer.width - scrollWidth - padHoriz;
            ...
            var hasScrollbar =  maxHeight >= rectContainer.height;
            var contentWidth = hasScrollbar ? minWidth : rectContainer.width - padHoriz;
            var rectContent  = new Rect( -padLeft, -rectView.y, contentWidth, maxHeight);

            // draw content
            
            rectContent.height=ContentHeight(rectView.width) +200 ;
            using( var scroll = new GUI.ScrollViewScope( rectView, mScrollPos, rectContent ) )
            {
                mScrollPos = scroll.scrollPosition;
